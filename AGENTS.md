...
- **As of 2026-06-29 (Claude review + Rose audit + merge of the small-sample interval calibration scaffold; HSquared.jl `main` `5e37cb53`/#192; hsquared R twin clean `main` `8c5c886`/#112).**
  Picked up the Codex handover branch `codex/small-sample-interval-calibration`, rehydrated, and found the scaffold PR was already pushed/opened as **#192** (DRAFT) — the handover's "still needs to be pushed/opened" was stale. Verified **CI 4/4 green** (Julia 1.10 + Julia-latest + docs + documenter/deploy) and ran a **real Rose claim-vs-evidence audit** (`rose-systems-auditor` subagent) that independently re-verified every fence and load-bearing number: no `src/`/`R/` touched, `validation_status()` unchanged (**48 rows, planned=1, covered=5**), `V1-HERIT-TCAL` stays **planned** (a docs-register row, NOT a `validation_status()` data row), the harness is a pure `using HSquared` consumer (t/SW probes script-local, surfaced only as `_probe` labels; the `df_eff<2→nothing` guard prevents a false SW "win"), triage TSVs/df values/line counts reconcile, and the two foreign files are unstaged — **verdict clean-with-limitations, no required changes**. On maintainer go-ahead, squash-**merged #192** to `main` (`5e37cb53`) and deleted the branch; did NOT auto-merge. **Nothing promoted to covered** (public default still v0.1 Gaussian). This bullet + the close-out PR refresh the snapshot and bank the review/audit/merge after-task. Two foreign untracked files remain never-stage. **START HERE:** `docs/dev-log/after-task/2026-06-29-pr192-review-merge-closeout.md`.

- **As of 2026-06-29 (Codex small-sample interval calibration branch + Claude handover; HSquared.jl branch `codex/small-sample-interval-calibration`, commits `d7effc79` + `6581828f`, hsquared R twin clean `main` `8c5c886`/#112, **merged to `main` via #192 (`5e37cb53`)**).**
