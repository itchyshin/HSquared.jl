...
| **A32 (S6)** | **PARKED (licence ABSENT; design freeze intact; not pushed)** | S6 ASReml comparator design frozen: Julia `990e33c6` (pre-declaration + skeleton) + `34dedc0a` (check-log). **One document, two independently gated legs** — Leg E estimand agreement at tail scale (closes debt item (2)); Leg W wall-clock ladder (closes **no** debt). Agreement gates timing per cell: a cell that fails agreement yields **no** timing number. Seven-cell grid indexed on `(q, fill, class)`, not `n`. Caps, cold-start seeds (`202696xx`/`202697xx`), and a **toolchain assertion** (the S5 lesson — it recorded its interpreter without asserting it) all frozen. Skeleton `sim/phase_s6_asreml_wallclock_ladder.jl` refuses to run without ASReml and has no HSquared-only fallback; only its dry-run and refusal path were exercised. **Prerequisites:** P1 **ABSENT** (A33); P2 ASReml scaffold **PORTED 2026-09-02** with provenance (prepare/run/f0; foreign tip `853bcc12`, introducing `29d04a1d`/`533cf0f8`) — **NOT RUN**; P1 ABSENT; P3 `fit_eigen_reml` is **absent from `src/` on this branch**, so the spine's three-fitter grid is not executable here as written. **No run, no compute, no speed claim** — the honest answer stays *cannot say*. `public_covered_count` **5**; `V1-MATFREE-REML` **experimental**. LOOP `~/local-scratch/h2-s6-LOOP.md`; briefing §24. |
| **A32b (S6 P2)** | **ported, NOT RUN (not pushed)** | Provenance-port of ASReml scaffold + high-fill generator from foreign tip `853bcc12` (`codex/2026-07-13-v07-performance-localization`, READ ONLY): `comparator/prepare_asreml_matfree.jl`, `comparator/run_asreml_matfree.R` (introducing `29d04a1d`, 2026-07-28), `sim/drac/f0_adversarial_fill.jl` (introducing `533cf0f8`, 2026-07-24). Bodies blob-matched; provenance headers. Skeleton dry-run exit 0 (scaffold `present`); refusal still stops on P1. P3 eigen ABSENT handling restated from pre-declaration. **Grid NOT RUN.** Check-log `check-log.d/2026-09-02-h2-s6-asreml-scaffold-port.md`. `public_covered_count` **5**. |
| **A33 (S6 P1)** | **DONE — ABSENT (not pushed)** | Licence probe: Mac R 4.6.0 + Totoro R 4.5.3 via ControlMaster — `asreml` not installed on either host (never reached a licence-error path). Receipt `~/local-scratch/h2-a33-asreml-licence-probe.md`. **S6 ladder PARKED**; A32/A32b design freeze intact; **no speed claim**, no grid, no install. A34/A35 unarmed until PRESENT re-probe. Check-log `check-log.d/2026-09-02-h2-a33-asreml-absent-s6-parked.md`. `public_covered_count` **5**. |
| **Owner asks (open)** | **OPEN** | (1) Bridge start values: keep shipped `I₂` vs adopt engine `0.5·phenotypic-variance` default (measured cost of identity ≈ 2e-5 on G0). (2) ~~**DP-1**~~ **DONE** — draft PRs #277 / #141; **CI GREEN on PR heads** (Julia `c8a4620a`: CI `33622314629` + Documenter `33622314613`; R `02a92a5`: R-CMD-check `33619570481`). Drafts ready for owner review; **no merge**. (3) ~~**A33**~~ **DONE ABSENT** — S6 PARKED; obtain-licence-elsewhere remains optional owner ask with S7/D1, not an agent track. (4) **A25 MV-5 disposition** — recommended default now **SUPERSEDE**, see `~/local-scratch/h2-a25-mv5-disposition-draft.md`. |
