...
the existing supplied-precision estimator, but it remains partial and held. A
432-seed diagnostic pilot over the preregistered cells, generated by the
pre-repair Julia-only harness, stopped on three low-convergence cells and two
precision blockers, so confirmation did not launch. It is not end-to-end
cross-twin recovery; Rose review and G10 therefore cannot activate it. Production
...
- Most public model syntax remains planned. The R twin has an executable narrow
  genomic activation candidate, now held after its diagnostic pilot stopped; it is
  not an activated default and has no Rose/G10 promotion basis.
...
the R twin has a matching narrow `genomic()` candidate. A 432-seed diagnostic
pilot over the preregistered cells, generated by the pre-repair Julia-only
harness, stopped: three cells were below 95% convergence and two more exceeded
the 2,000-replicate precision cap, so confirmation was not run and
...
`public_covered_count` remains 5. Pilot evidence and the exact stop decision are
banked in `docs/dev-log/recovery-checkpoints/2026-07-12-v07-genomic-activation-pilot-stop.md`.
