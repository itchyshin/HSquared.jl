...
# The kernel this harness was written against (arc S2b). A run that deliberately measures a
# DIFFERENT kernel passes its sha in HSQ_EXPECTED_KERNEL_SHA, so the check still fails loudly
# on a stale checkout but does not block an intended re-measurement.
const EXPECTED_KERNEL_SHA = get(ENV, "HSQ_EXPECTED_KERNEL_SHA",
                                "8212dee2ce76e92a9b2815c499294c3bfb593f70")

function _assert_kernel_sha!()
    sha = _kernel_commit_sha()
    sha == EXPECTED_KERNEL_SHA ||
        error("precondition failed: src/takahashi_selinv.jl last commit is $(sha), expected " *
              "$(EXPECTED_KERNEL_SHA) (set HSQ_EXPECTED_KERNEL_SHA to measure a different kernel on purpose)")
    return sha
end
...
# ---------------------------------------------------------------------------
# Thread sweep on the banked point (added 2026-09-19, Szymek).
#
# D-271's numbers were all taken at ONE BLAS thread. Our kernel is scalar and
# single-threaded; SelectedInversion.jl rides CHOLMOD's supernodal BLAS-3, which threads.
# On a Mac Studio M1 Ultra the package's lead over our post-#361/#363 kernel went 4.5x at
# 1 thread -> 10.6x at 4 -> 13.9x at 8, i.e. it crosses D-271's 10x bar somewhere between 1
# and 4 threads. Which side of the bar the decision lands on therefore depends on a thread
# count the rule never fixed. This mode measures that curve on the banked fixture.
#
#   arm a_trace   HSquared.selinv_trace_against  (the DEFAULT kernel)
#   arm a_strict  HSquared._selinv_zvals(F; strict_order = true)  (bit-identical kernel;
#                 absent on kernels older than #363, skipped automatically)
#   arm a_diag    HSquared.takahashi_diag
#   arm c_trace   SelectedInversion.selinv + dot against the permuted Ainv
#   arm c_diag    SelectedInversion.selinv_diag
#
# HSQ_THREAD_SWEEP  comma-separated thread counts   (default "1,2,4,8,16,32,64")
# HSQ_TOTORO_Q      fixture size                    (default 20000, the banked point)
# HSQ_SWEEP_REPS    timed repeats per arm           (default 2)
# ---------------------------------------------------------------------------

_supports_strict_order() =
    any(m -> :strict_order in Base.kwarg_decl(m), methods(HSquared._selinv_zvals))

function totoro_thread_sweep()
    sha = _assert_kernel_sha!()
    threads = parse.(Int, split(get(ENV, "HSQ_THREAD_SWEEP", "1,2,4,8,16,32,64"), ","))
    q = parse(Int, get(ENV, "HSQ_TOTORO_Q", "20000"))
    reps = parse(Int, get(ENV, "HSQ_SWEEP_REPS", "2"))
    strict_ok = _supports_strict_order()

    spec = f0adv_fixture(q; nfounder_frac = 0.005)
    Ainv = sparse(Float64.(spec.Ainv))
    nfixed = size(spec.X, 2)
    n = size(spec.X, 2) + size(spec.Z, 2)
    lhs, _, _ = HSquared._sparse_mme_system(spec, 1.0, 1.0)
    ...
    path = joinpath(@__DIR__, "results",
                    "selinv_threads_$(_harness_commit_sha())_q$(q).tsv")
    open(path, "w") do io
        for l in _header_lines(); println(io, l); end
        println(io, "# THREAD SWEEP on the banked fixture. kernel_sha=$(sha) ",
                    "strict_order_available=$(strict_ok) fill=$(round(fill, digits = 3)) ",
                    "is_super=$(_is_super(F)) reps=$(reps)")
        println(io, join(["threads", "q", "nnz_L", "fill", "is_super", "T_fact",
                          "T_a_trace", "T_a_strict", "T_a_diag", "T_c_trace", "T_c_diag",
                          "S_c_trace", "S_c_diag", "err_c"], "\t"))
        for t in threads
            BLAS.set_num_threads(t)
            cholesky!(F, lhs_sym; check = true)                       # warm-up + refresh
            T_fact = _median([(@timed cholesky!(F, lhs_sym; check = true)).time for _ in 1:3])

            tr_a = HSquared.selinv_trace_against(F, Ainv, nfixed)     # warm-up
            T_a_trace = _median([(@timed HSquared.selinv_trace_against(F, Ainv, nfixed)).time
                                 for _ in 1:reps])
            T_a_strict = NaN
            if strict_ok
                HSquared._selinv_zvals(F; strict_order = true)
                T_a_strict = _median([(@timed HSquared._selinv_zvals(F; strict_order = true)).time
                                      for _ in 1:reps])
            end
            HSquared.takahashi_diag(F)
            T_a_diag = _median([(@timed HSquared.takahashi_diag(F)).time for _ in 1:reps])

            Zp = SelectedInversion.selinv(F; depermute = false)       # warm-up
            tr_c = LinearAlgebra.dot(Zp.Z, B_perm)
            T_c_trace = _median([(@timed begin
                                      Z = SelectedInversion.selinv(F; depermute = false)
                                      LinearAlgebra.dot(Z.Z, B_perm)
                                  end).time for _ in 1:reps])
            SelectedInversion.selinv_diag(F)
            T_c_diag = _median([(@timed SelectedInversion.selinv_diag(F)).time for _ in 1:reps])

            err_c = abs(tr_a - tr_c) / max(abs(tr_c), eps())
            println(io, join(string.([t, q, nnzL, round(fill, digits = 3), _is_super(F),
                                      round(T_fact, digits = 6),
                                      round(T_a_trace, digits = 6), round(T_a_strict, digits = 6),
                                      round(T_a_diag, digits = 6), round(T_c_trace, digits = 6),
                                      round(T_c_diag, digits = 6),
                                      round(T_a_trace / T_c_trace, digits = 2),
                                      round(T_a_diag / T_c_diag, digits = 2),
                                      @sprintf("%.3e", err_c)]), "\t"))
            flush(io)
            @printf("threads=%3d  fact %.3f s | ours trace %.3f s  strict %.3f s  diag %.3f s | pkg trace %.3f s  diag %.3f s | package faster by %.2fx (trace) %.2fx (diag)  err %.2e\n",
                    t, T_fact, T_a_trace, T_a_strict, T_a_diag, T_c_trace, T_c_diag,
                    T_a_trace / T_c_trace, T_a_diag / T_c_diag, err_c)
            flush(stdout)
        end
    end
...
function main(args)
    ...
    if length(args) >= 1 && args[1] == "--thread-sweep"
        return totoro_thread_sweep()
    end
