...
# against de Villemereuil's QGglmm package (CRAN 0.8.0) `QGparams(..., model=<model>)$h2.obs`.
#
# Covers the four engine observation-scale families that map onto a QGglmm built-in model:
#   • :bernoulli (logit)    ↔ binom1.logit          • :bernoulli_probit ↔ binom1.probit
#   • :poisson (log, count) ↔ Poisson.log           • :binomial(n>1)    ↔ binomN.logit (n.obs = n)
#
...
#   CSV rows: `mu,V_A,V_fixed,model,n_obs,engine_h2_observation`
#   (n_obs used only for binomN.*; leave empty/NA otherwise)
...
maxdiff <- 0
cat(sprintf("%-16s %-15s %-5s %-13s %-13s %-9s\n", "mu,V_A,V_fixed", "model", "n", "engine", "QGglmm", "abs_diff"))
for (i in seq_len(nrow(d))) {
  mu <- as.numeric(d[i, 1]); va <- as.numeric(d[i, 2]); vf <- as.numeric(d[i, 3])
  model <- trimws(d[i, 4]); nobs <- suppressWarnings(as.numeric(d[i, 5])); e <- as.numeric(d[i, 6])
  r <- if (grepl("^binomN", model)) {
    QGparams(mu = mu, var.a = va, var.p = va + vf, model = model, n.obs = nobs, verbose = FALSE)
  } else {
    QGparams(mu = mu, var.a = va, var.p = va + vf, model = model, verbose = FALSE)
  }
  df <- abs(r$h2.obs - e); maxdiff <- max(maxdiff, df)
  cat(sprintf("%-16s %-15s %-5s %-13.7f %-13.7f %.2e\n",
              paste(mu, va, vf, sep = ","), model, ifelse(is.na(nobs), "-", nobs), e, r$h2.obs, df))
}
cat(sprintf("\nmax |engine - QGglmm| = %.2e  (threshold 1e-4)\n", maxdiff))
if (maxdiff < 1e-4) {
  cat("PASS: engine observation-scale h2 == QGglmm (binom1.logit/probit, Poisson.log, binomN.logit)\n")
  quit(status = 0)
} else {
