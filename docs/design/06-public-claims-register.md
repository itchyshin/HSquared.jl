...
| Henderson MME equations can be solved at supplied variance components | partial | shared R/Julia supplied-variance fixture pins Ainv, fixed effects, EBVs, fitted values, and `h2`; R head `ca8bce1` also compares Julia against an independent R MME reference when available | sparse supplied-variance equation solve; not variance-component estimation, fitted Mrode validation, external fitted-model parity, or production fitting |
| Dense EBV/BLUP, heritability, reliability, and PEV extractors | partial | `breeding_values()`, `heritability()`, `prediction_error_variance()`, and `reliability()` tiny hand-checked tests plus Henderson MME fixture; `variance_components()`, `heritability()`, PEV, and reliability also cover supplied-variance `HendersonMMEResult` objects | experimental dense low-level extractors; not production sparse EBVs, reliability, or PEV |
