...
| `data_status()` reports `HSData` diagnostics | covered | `data_status()` tests for component presence, ID-overlap rows, checked marker alignment, marker-only, genotype-only, and phenotype-only cases; R head `1fe0f4c` has matching R-side diagnostics | diagnostic only; no bridge payload change, genotype parsing, relationship construction, marker scanning, genomic fitting, or QTL/eQTL fitting |
...
| R `hs_data()` can feed the v0.1 parser | external covered | R head `36efbf3`; R-CMD-check `27460091544`, pkgdown `27460091551`, Pages `27460131691` green | R parser can read phenotypes and pedigree objects from `hs_data()` for the v0.1 preview/validation contract; no file-backed storage, genotype/omics automatic model construction, production bridge hardening, or general fitting claim |
