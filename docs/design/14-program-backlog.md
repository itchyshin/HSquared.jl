...
- **H7** ✅ Latent- & observation-scale non-Gaussian h² (Nakagawa–Schielzeth / de Villemereuil) — engine half. [JL] (NEW EXPORT `nongaussian_heritability`; latent h² = V_A/(V_A+V_link+V_fixed), observation h² = Ψ²·V_A/V_P,obs via the existing 20-node GH (logit, proportion) / log-normal closed form (Poisson, count); corrected the estimand — π²/3 is NOT in the observation-scale integration variance; oracle = Gaussian reduction + logit latent closed form + observation vs independent 64-node quadrature + Poisson closed form + n_trials gradient + guards; `V6-NS-H2` row + .md mirrors; partial; PR `claude/h7-nongaussian-h2`. Same-estimand QGglmm/MCMCglmm comparator + Fisher/Falconer sign-off + R bridge are the remaining `[bridge]` follow-on.)
