...
| `HSData` input container | implemented | in-memory phenotype/pedigree/genotype/expression ID-map tests plus marker-map metadata, genotype-marker alignment, pedigree-status diagnostics, and `data_status()` diagnostic tests; no file-backed storage, genotype parsing, marker scanning, genomic relationship construction, Ainv construction from raw data containers, or modelling claim |
...
| MME-backed EBVs/BLUPs | experimental | `breeding_values(fit)` now uses the Henderson MME solve at the fit's variance components; hand-checked identity-relationship and Henderson MME fixture tests cover values and MME equivalence; no production sparse fitting or external fitted-model comparator yet |
| Dense heritability | experimental | `heritability()` hand-checked simple univariate variance-ratio test |
| Validation-scale supplied-variance outputs | experimental | `variance_components()`, `heritability()`, `prediction_error_variance()`, and `reliability()` tested for supplied-variance `HendersonMMEResult` objects; PEV/reliability use dense Henderson MME inverse blocks; no sparse production solve or comparator yet |
...
| R `hs_data()` data container | external implemented | `hsquared` head `644c75e`; head `36efbf3` connects `hs_data()` to the v0.1 R parser; Julia mirror exists as `HSData`, but live Julia object marshalling is not implemented |
