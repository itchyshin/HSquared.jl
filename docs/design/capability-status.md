...
| R v0.1 formula parser and payload builder | external implemented | `hsquared` head `b57b48e`; R parser builds the narrow bridge payload; head `9eabf0d` adds opt-in experimental `hs_control(engine = "julia")` for tiny/local use; head `398e019` records sparse `Z` marshalling through Julia `sparse_csc_matrix()` |
