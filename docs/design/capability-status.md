...
| `HSData` input container | implemented | in-memory phenotype/pedigree/genotype/expression ID-map tests plus marker-map metadata, genotype-marker alignment, and `data_status()` diagnostic tests; no file-backed storage, genotype parsing, marker scanning, genomic relationship construction, or modelling claim |
