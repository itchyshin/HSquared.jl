...
| MME-backed EBVs/BLUPs and fitted values | experimental | `breeding_values(fit)` and `fitted_values(fit)` now use the Henderson MME solve at the fit's variance components; hand-checked identity-relationship and Henderson MME fixture tests cover values and MME equivalence; no production sparse fitting or external fitted-model comparator yet |
