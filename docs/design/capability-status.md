...
| `HSData` input container | implemented | in-memory phenotype/pedigree/genotype/expression ID-map tests plus marker-map metadata and genotype-marker alignment tests; no file-backed storage, genotype parsing, marker scanning, genomic relationship construction, or modelling claim |
