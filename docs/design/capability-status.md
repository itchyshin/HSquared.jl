...
| R `hs_data()` data container | external implemented | `hsquared` head `644c75e`; head `36efbf3` connects `hs_data()` to the v0.1 R parser; head `e7fbb31` adds environment-key diagnostics; Julia mirror exists as `HSData`, but live Julia object marshalling is not implemented |
