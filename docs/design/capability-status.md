...
| Validation-scale reliability / PEV | experimental | `prediction_error_variance()` and `reliability()` tested for dense `AnimalModelFit` and supplied-variance `HendersonMMEResult` objects against dense Henderson MME inverse blocks; no sparse production solve or comparator yet |
