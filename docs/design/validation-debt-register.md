...
| V1-DATA | HSData in-memory input container | covered | phenotype, pedigree, genotype, expression ID-map tests plus marker-map metadata and genotype-marker alignment failure-mode tests in `test/runtests.jl`; no genotype parsing or genomic modelling claim |
