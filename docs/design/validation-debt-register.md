...
| V1-BRIDGE | R-to-Julia payload parity | partial | R payload builder exists in `hsquared` head `b57b48e`; Julia direct payload target has spec-dispatch, parent-index, `ids`, sparse `Z`, and Julia-side `Ainv` parity tests; R head `c837f2d` adds an internal JuliaCall smoke; still needs sparse marshalling and public engine-control tests |
