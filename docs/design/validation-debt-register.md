...
| V1-DATA | HSData in-memory input container | covered | phenotype, pedigree, genotype, expression ID-map tests plus marker-map metadata, genotype-marker alignment failure-mode, and `data_status()` diagnostic tests in `test/runtests.jl`; no genotype parsing or genomic modelling claim |
