...
Resolve PR #22 (`codex/phase5-marker-plot-data`) against the repaired
`codex/phase5-marker-lod` base after PR #20 was reconciled with the Phase 5
marker-scan stack.
...
- Fisher: preserve marker-scan interpretation boundaries.
- Grace: verify low-core local checks before push.
- Rose: prevent plotting, mixed-model GWAS/QTL, LOCO, or bridge claims.
- Spawned agents: none.
...
`codex/phase5-marker-plot-data`. The only conflict was the append-only
`docs/dev-log/check-log.md`; the resolution keeps:

- the PR #22 fixed-effect marker-scan Manhattan plot-data evidence;
...
`result_payload()`, R package files, plotting implementation, or public claims
...
Allowed claim: PR #22 is reconciled locally against the repaired PR #20 base
while preserving its deterministic fixed-effect marker-scan Manhattan
plot-data output.

Blocked claims remain blocked: no plotting implementation, no marker-map
parser, no mixed-model marker scan, no GWAS/QTL/interval-mapping claim, no
LOCO, no calibrated correlated-marker or genome-wide claim, no R formula
