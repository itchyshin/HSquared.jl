...
| 7 | The SMOKE mode of slice C's new driver (`sim/f6_matfree_recovery.jl`) reported a meaningless FAIL at `nm=60` — provenance took three rounds to settle precisely (Rose's report → this audit's flag → slice-C's partial counter → `main`'s final correction) | evidence/verification | **adaptive, settled** (was: drift-in-the-closeout; downgraded per `main`'s correction) | closed — `main` + `rose-closeout` | **Settled sequence, per `main`:** slice-C saw `rel_a=979.8` in her own local SMOKE run, judged it acceptable against F5-v2 precedent, and did **not** fix it → Grace's Totoro SMOKE run on the same file then reported the spurious `GATE: FAIL (exit 1)` to `main` → `main` applied the fix (scale-relative `on_boundary`/`safe_rel` + plumbing-only SMOKE) and verified on both Julia versions before anything was committed. `git log --all --oneline -- sim/f6_matfree_recovery.jl` (independently verified, one commit only) confirms the load-bearing part regardless of exact mechanism: nothing defective ever shipped. Both `slice-C`'s "no Totoro involvement" (she misread the handover's S8-OPEN text as post-session state; it was Szymek's pre-session text) and Rose's original "found by a Totoro probe" (omitted that slice-C saw it first) were imprecise, not fabricated. **One residual precision note, not reopened as a dispute:** `main`'s corroborating citation for "the Totoro probe definitely ran" ("fill 583.3, CONVERGED in 59 iterations, 54.26s at q=25,000") is the *separate*, already-documented S5 tail-scale feasibility probe from `33ab68f6` — same host, different script/scale/purpose than the `nm=60` SMOKE check. Doesn't contradict the settled sequence (which rests on `main`'s direct first-hand account of receiving Grace's report and doing the fix herself), just noted for the record rather than silently absorbed. `rose-closeout` independently re-verified and retracted the original Finding 6 wording in `3e00eaae` (committed separately, not amended). |
...
- **Row 7** (SMOKE provenance): `main` supplied the first-hand settled sequence and it is recorded
  above, with one residual citation-precision note that doesn't change the substance. Tag downgraded
  `drift (in the closeout report itself)` → `adaptive, settled`. `rose-closeout` independently
  re-verified and retracted her own original wording in a separate commit, `3e00eaae`, rather than
  amending — consistent with this repo's own non-destructive-correction convention.

No row's underlying technical substance changed as a result of this addendum — `public_covered_count`
stays 5, no capability flip, nothing shipped defective, all eight GOAL fences held throughout. Only
attribution/provenance precision changed.
