...
| 7 | The SMOKE mode of slice C's new driver (`sim/f6_matfree_recovery.jl`) reported a meaningless FAIL at `nm=60`; the bug/fix/no-impact substance is undisputed across every account, but the exact discovery mechanism has now gone through five rounds without independent confirmation | evidence/verification | **substance: settled. mechanism: PROVISIONAL, reopened** | open — `rose-closeout` verifying directly with `totoro-smoke` | **What's settled, undisputed by any account:** slice-C saw `rel_a≈980` in her own local SMOKE run, judged it acceptable against F5-v2 precedent, did not fix it; exactly one commit (`7ceaff17`) ever touched the file, already carrying the fix; verified on both Julia versions; no impact. **What's NOT settled:** whether Totoro's SMOKE run is what surfaced it to the orchestrator. `c0d35d5e`'s cited "primary evidence" (a terminal transcript, `host=totoro julia=1.10.0`, `exact: sigma_a2=0.0000 sigma_e2=0.7044 ... GATE: FAIL`) exists **only pasted inline in that commit's own message** — this audit searched the repo for a standalone artifact (log file, TSV, any `*totoro*` output) and found **none**. A quoted transcript in a commit message is not self-authenticating, and this audit initially treated it as settling the question because it neutralized this audit's own prior (also invalid) git-log argument — **that was a second error, not a correction of the first: showing a counter-argument is invalid does not make the original claim true, it only removes one piece of evidence against it.** `rose-closeout` caught both errors — the misattribution (`c0d35d5e` is the orchestrator's commit, not hers; corrected upstream in `86e59a04`) and this deeper one — and is checking directly with `totoro-smoke` for first-hand confirmation or the actual log, same standard this audit should have held `c0d35d5e` to from the start. Downgraded from "settled (final)" pending that reply. The general lesson from the git-log round still stands on its own regardless of this instance's outcome: **absence from `git log` is not absence from the session** — but that is a statement about what git-log CANNOT disprove, not evidence for what actually happened. |
...
**Standing lesson, now in two parts, both owned by this audit:** (1) row 7's original text treated
`git log --all` returning a single commit as proof the defective version was never run anywhere —
invalid, since git records commits, not working-tree or remote-host state. (2) Round 4 then treated
"the counter-argument against a claim is invalid" as equivalent to "the claim is verified" — also
invalid; a quoted transcript with no independent artifact is exactly the same evidentiary category as
the "found via a Totoro probe" phrase that started this whole thread. Both are noted here rather than
in `PLAN-DRIFT-LEDGER.md` (that ledger aggregates recurring classes monthly; this is one instance of
each so far), but both are exactly the shape of thing to watch for a second occurrence of.

No row's underlying **technical** substance has changed at any point — `public_covered_count` stays 5,
no capability flip, nothing shipped defective, all eight GOAL fences held throughout, and the
bug/fix/no-impact facts are undisputed by every account including the still-open one. Five rounds so
far have moved only attribution/provenance precision on one low-stakes detail — genuinely open, not
performatively so, until `totoro-smoke` answers.
