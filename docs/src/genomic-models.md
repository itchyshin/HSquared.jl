...
permission to activate the R default: a 432-seed diagnostic pilot over the
preregistered cells, generated by the pre-repair Julia-only harness, stopped on
three low-convergence cells and two precision blockers. Confirmation was not
run and this is not end-to-end cross-twin recovery. The R surface therefore remains
partial/held with no Rose/G10 promotion basis. `single_step()` remains a separate experimental surface.
...
- repair/localize the optimizer failures found by the stopped diagnostic pilot
  before any fresh end-to-end R recovery attempt; the narrow R `genomic()`
  candidate stays held and `single_step()` remains separate;
