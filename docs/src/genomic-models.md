...
identity evidence. It is **not** broad recovery, production-scale evidence, or
permission to activate the R default: the R surface remains partial pending the
preregistered recovery campaign, independent reviews, Rose audit, and G10
maintainer decision. `single_step()` remains a separate experimental surface.
...
- merge and activation of the narrow R `genomic()` candidate, conditional on
  recovery, independent audits, Rose review, and G10; `single_step()` remains
  separate;
- an independent base-R reconstruction of the exact candidate kernel and a
  broader construction oracle. A fresh hash-pinned `blupf90+` run now links the
