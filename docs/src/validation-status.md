...
| `V5-MARKER-FIXED` | fixed-effect single-marker scan | Phase 5 | partial | Fixed-effect Gaussian screening utility only, using supplied residual variance and approximate Wald p-values; no mixed-model GWAS/QTL claim, no R formula term activation, no bridge payload change, and no comparator evidence. |
