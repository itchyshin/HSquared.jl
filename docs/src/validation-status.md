...
| `V4-MV-REML` | multivariate REML (genetic/residual covariance estimation) | Phase 4 | partial | Experimental dense/validation-scale multivariate REML; correctness is self-consistency + univariate-reduction validated and adversarial-reviewed (robustness findings fixed), copy-returning Julia accessors wrap existing fields without widening `result_payload()`, an opt-in seeded recovery harness exists outside CI with explicit seed-list reporting, and a serialized Julia target fixture plus comparator protocol exists for R-lane parity work; the recovery calibration protocol was executed and did not pass (unstructured 6/10, all fits converged; failure-mode triage found 3 G-only failures plus 1 G+R failure), so recovery is not broadly multi-seed calibrated and has no external-comparator parity yet; not the public default and no R-facing model-spec. |
...
| `V5-MARKER-FIXED` | fixed-effect single-marker scan | Phase 5 | partial | Fixed-effect Gaussian screening utility with marker-effect summary, marker-map-backed Manhattan, QQ, and inflation diagnostic helpers only, using supplied residual variance plus approximate Wald p-values, Bonferroni/BH adjustments, LOD-equivalent scores, Manhattan data, QQ data, and a lambda_GC diagnostic over the returned marker set; no mixed-model GWAS/QTL claim, no p-value calibration claim, no R formula term activation, no bridge payload change, and no comparator evidence. |
...
unstructured REML row now has an opt-in seeded recovery harness with explicit
seed-list reporting and a serialized two-trait target fixture plus comparator
protocol for R-lane comparator work, but no sommer, ASReml, JWAS, or BLUPF90
multi-trait parity is claimed. The
