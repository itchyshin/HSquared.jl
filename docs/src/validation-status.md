...
| `V5-GENOMIC-QTL` | genomic, marker, QTL, and eQTL validation | Phase 5 | planned | Broad genomic/QTL/eQTL validation remains planned; the direct fixed-effect `single_marker_scan` utility is tracked separately in `V5-MARKER-FIXED`, and the direct supplied-variance `mixed_model_marker_scan` utility is tracked separately in `V5-MARKER-MIXED`. |
