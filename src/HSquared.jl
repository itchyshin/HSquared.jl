module HSquared

using LinearAlgebra
using Optim
using Random
using SHA
using SparseArrays

export AMDGPUBackend,
    AutoBackend,
    AnimalModelFit,
    AnimalModelSpec,
    BackendInfo,
    BackendInfoRow,
    BreedingValues,
    BLUP,
    CPUBackend,
    CUDABackend,
    EBV,
    FormulaStatus,
    FormulaStatusRow,
    GaussianFamily,
    GaussianLikelihoodResult,
    HendersonMMEResult,
    HSControl,
    HSAnnotationSpec,
    HSData,
    HSDataExpressionStatusRow,
    HSDataGenotypeStatusRow,
    HSDataIDMap,
    HSDataIDOverlapRow,
    HSDataAnnotationStatusRow,
    HSDataEnvironmentStatusRow,
    HSEnvironmentSpec,
    HSDataMarkerStatusRow,
    HSDataPedigreeStatusRow,
    HSDataStatus,
    MetalBackend,
    OneAPIBackend,
    Pedigree,
    Phase0NotImplementedError,
    ThreadsBackend,
    ValidationStatus,
    ValidationStatusRow,
    accuracy,
    animal_model_spec,
    backend_info,
    bootstrap_variance_component_interval,
    breeding_values,
    centered_markers,
    common_env,
    comparable_loglik,
    cytoplasmic,
    diagonal_covariance,
    dominance,
    epistasis,
    factor_analytic_covariance,
    FA_UNIQUENESS_FLOOR,
    fa_covered_flip_cell,
    fit_ai_reml,
    fit_animal_model,
    fit_diagnostics,
    fit_laplace_reml,
    laplace_reml_interval,
    ledermann_slack,
    fit_gblup,
    fit_gblup_reml,
    fit_multivariate_reml,
    fit_multi_effect_reml,
    fit_sparse_multi_effect_aireml,
    multi_effect_ratio_interval,
    fit_direct_maternal_reml,
    direct_maternal_interval,
    fit_repeatability_reml,
    repeatability_interval,
    fit_snp_blup,
    fit_snp_blup_reml,
    single_step_inverse,
    fit_single_step,
    fit_single_step_reml,
    metafounder_single_step_inverse,
    fit_metafounder_single_step,
    fit_metafounder_single_step_reml,
    fit_two_effect_reml,
    two_effect_ratio_interval,
    fit_sparse_reml,
    fit_variance_components,
    formula_status,
    fitted_values,
    fixed_effects,
    gaussian_loglik,
    covariance_structure_lrt,
    nested_lrt,
    genetic_correlation,
    genetic_correlation_interval,
    genetic_loadings,
    genetic_structure,
    genetic_uniqueness,
    evolvability,
    conditional_evolvability,
    respondability,
    autonomy,
    variance_along_gradient,
    genetic_pca,
    g_max,
    mean_evolvability,
    genetic_gllvm_descriptors,
    genetic_gllvm_gaussian_mme,
    genetic_pca_plot_data,
    genetic_correlation_plot_data,
    variance_components_plot_data,
    breeding_values_plot_data,
    hsquared_figure,
    legendre_basis,
    legendre_design,
    random_regression_mme,
    fit_random_regression_reml,
    standardize_covariate,
    rr_genetic_variance,
    rr_genetic_covariance_surface,
    rr_genetic_correlation_surface,
    rr_heritability,
    rr_eigenfunctions,
    rr_eigenfunctions_plot_data,
    rr_genetic_variance_plot_data,
    rr_covariance_surface_plot_data,
    multivariate_covariance_standard_errors,
    genome_wide_pvalue,
    genome_wide_threshold_from_null,
    genome_wide_marker_scan,
    genomic,
    genomic_relationship_inverse,
    apy_genomic_relationship_inverse,
    genomic_relationship_matrix,
    gpu_fit_gblup,
    gpu_genomic_relationship_inverse,
    gpu_genomic_relationship_matrix,
    gwas_table,
    henderson_mme,
    solve_animal_model_pcg,
    solve_multi_effect_pcg,
    mc_reml_block_traces,
    fit_multi_effect_mc_reml,
    fit_matrix_free_reml,
    matrix_free_reml_loglik,
    matrix_free_reml_information,
    matrix_free_ratio_intervals,
    fit_multi_effect,
    heritability,
    heritability_interval,
    heritability_standard_error,
    hsquared,
    data_status,
    id_map,
    eqtl_table,
    imprinting,
    inbreeding_coefficients,
    maternal_lineage,
    mendelian_sampling_variances,
    additive_relationship,
    cytoplasmic_relationship,
    clonal_relationship,
    dominance_relationship,
    epistatic_relationship,
    metafounder_relationship,
    metafounder_relationship_inverse,
    metafounder_inverse,
    metafounder_inbreeding,
    metafounder_animal_model,
    lowrank_covariance,
    LOGLIK_CONVENTION_FULL,
    LOGLIK_CONVENTION_OMIT_2PI,
    loco_relationship_precisions,
    loco_mixed_model_marker_scan,
    marker_scan,
    marker_effects,
    marker_genomic_inflation,
    marker_manhattan_data,
    marker_region_data,
    marker_scan_result_payload,
    marker_scan_table,
    marker_significance_summary,
    marker_variance_explained,
    mixed_model_marker_scan,
    marker_qq_data,
    markers,
    maternal_env,
    multivariate_mme,
    multivariate_result_payload,
    structured_genetic_payload,
    maternal_genetic,
    nongaussian_heritability,
    nongaussian_result_payload,
    normalize_pedigree,
    pedigree_inverse,
    planned_genomic_qtl_terms,
    permanent,
    planned_model_terms,
    planned_quantgen_terms,
    paternal_env,
    paternal_genetic,
    prediction_error_variance,
    qtl_scan,
    qtl_table,
    reml_full_constant_offset,
    reliability,
    require_fa_covered_flip_cell,
    relmat,
    multi_effect_mme,
    repeatability_mme,
    result_payload,
    parse_payload_v2,
    fit_payload_v2,
    result_payload_v2,
    single_marker_scan,
    single_step,
    sparse_reml_loglik,
    sparse_multi_reml_loglik,
    sparse_csc_matrix,
    two_effect_mme,
    validation_status,
    variance_component_covariance,
    variance_component_interval,
    variance_component_standard_errors,
    multi_effect_variance_component_covariance,
    multi_effect_variance_component_standard_errors,
    multi_effect_ratio_standard_errors,
    multi_effect_uncertainty,
    multi_effect_sum_ratio_interval,
    variance_components

include("backends.jl")
include("control.jl")
include("errors.jl")
include("pedigree.jl")
include("data.jl")
include("sparse_bridge.jl")
include("model_spec.jl")
include("genomic.jl")
include("takahashi_selinv.jl")
include("likelihood.jl")
include("iterative_solve.jl")
include("postfit.jl")
include("multivariate.jl")
include("evolvability.jl")
include("random_regression.jl")
include("nongaussian.jl")
include("genetic_gllvm.jl")
include("planned_terms.jl")
include("validation_status.jl")
include("bridge_payload_v2.jl")
include("placeholders.jl")
include("plotting_ext.jl")
include("gpu_ext.jl")

end
