const VALIDATION_STATUS_DATA = (
    ...
    (
        ...
        "Phase 4B",
        "partial",
        "`diagonal_covariance`, `lowrank_covariance`, and `factor_analytic_covariance` build `diag(σ²)`, `ΛΛ'`, and `ΛΛ' + Ψ` trait covariance matrices with finiteness/positivity guards. `fit_multivariate_reml(...; genetic_structure = :diagonal | :lowrank | :factor_analytic, rank = K)` reuses the dense multivariate REML core with constrained genetic covariance and unstructured residual covariance. Deterministic checks in `test/runtests.jl` pin constructor identities/guards, structure metadata, returned loglik equality to `_multivariate_reml_loglik`, PSD/PD covariance properties, and constrained fits not exceeding the unstructured REML loglik. Opt-in script `sim/phase4b_structured_covariance_recovery.jl` records seeded low-rank and factor-analytic recovery on a repeated-record half-sib design; it is not part of CI.",
        "loading sign/rotation convention, covariance SEs or LRTs, published multi-trait fixtures, multi-seed calibration, and sommer/ASReml/BLUPF90 comparator parity",
        "Experimental dense/validation-scale engine API only; opt-in recovery evidence is internal and seeded, no R-facing covariance-structure syntax, no bridge/result-payload change, no production sparse FA solver, and no external comparator evidence.",
    ),
    (
        "V5-GENOMIC-QTL",
