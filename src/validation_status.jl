const VALIDATION_STATUS_DATA = (
    ...
    (
        ...
        "Experimental dense/validation-scale multivariate REML; correctness is self-consistency + univariate-reduction validated and adversarial-reviewed (robustness findings fixed), Julia-side accessors wrap existing fields without widening `result_payload()`, an opt-in seeded recovery harness exists outside CI, and a serialized Julia target fixture plus comparator protocol exists for R-lane parity work, but recovery is not multi-seed calibrated and has no external-comparator parity yet; not the public default and no R-facing model-spec.",
    ),
    (
        "V4-FA",
