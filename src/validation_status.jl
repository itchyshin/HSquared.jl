const VALIDATION_STATUS_DATA = (
    ...
    (
        ...
        "Phase 4",
        "partial",
        "`fit_multivariate_reml` estimates `G0`, `R0` by dense multivariate REML (log-Cholesky-parameterized NelderMead on `V = Z(A⊗G0)Z' + block-diag R`, handling missing records). Correctness is pinned by deterministic checks in `test/runtests.jl`: the `t=1` reduction recovers the univariate REML estimate (`fit_sparse_reml`) to <1% on an interior-optimum fixture; the multivariate REML log-likelihood (`_multivariate_reml_loglik`) equals the univariate `sparse_reml_loglik` exactly at `t=1` (~1e-7) — it is the full REML loglik including the `(N−p')·log(2π)` constant, on the package-wide loglik scale; the fitted optimum beats a coarse `(G0,R0)` grid; and the returned EBVs equal `multivariate_mme` at the estimate. `test/fixtures/phase4_multitrait_parity/` serializes a deterministic two-trait parity fixture (pedigree, phenotypes, Julia REML target `G0`/`R0`, beta, EBVs, h², and loglik) for R-lane comparator work; CI checks fast self-consistency at the stored target covariances without re-running the optimizer. A one-off 12-replicate t=2 simulation (n=400 half-sib, not committed — suite is RNG-free) recovers the generating covariances on average (mean Ĝ0≈[1.03,0.61;0.61,2.25] vs [1.0,0.5;0.5,2.0]; mean R̂0≈truth; r̂g≈0.40 vs 0.354; 12/12 converged). A 7-lens adversarial review (each finding verified by running Julia) was run; its confirmed robustness findings — non-finite/empty-trait input guards and the REML loglik constant — are fixed and regression-tested.",
        "an RNG-based committed recovery harness, standard errors / likelihood-ratio tests for the covariances, a published Mrode multi-trait estimate, and JWAS/sommer/ASReml comparator parity",
        "Experimental dense/validation-scale multivariate REML; correctness is self-consistency + univariate-reduction validated and adversarial-reviewed (robustness findings fixed), and a serialized Julia target fixture exists for R-lane parity work, but multi-trait known-truth recovery is one-off only and has no external-comparator parity yet; not the public default and no R-facing model-spec.",
    ),
    (
        "V4-FA",
        ...
        "Phase 4B",
        "partial",
        "`diagonal_covariance`, `lowrank_covariance`, and `factor_analytic_covariance` build `diag(σ²)`, `ΛΛ'`, and `ΛΛ' + Ψ` trait covariance matrices with finiteness/positivity guards. `fit_multivariate_reml(...; genetic_structure = :diagonal | :lowrank | :factor_analytic, rank = K)` reuses the dense multivariate REML core with constrained genetic covariance and unstructured residual covariance. Deterministic checks in `test/runtests.jl` pin constructor identities/guards, structure metadata, returned loglik equality to `_multivariate_reml_loglik`, PSD/PD covariance properties, constrained fits not exceeding the unstructured REML loglik, and deterministic sign-canonicalization of returned loading columns. Opt-in script `sim/phase4b_structured_covariance_recovery.jl` records seeded low-rank and factor-analytic recovery on a repeated-record half-sib design; it is not part of CI.",
        "loading rotation/identifiability convention, covariance SEs or LRTs, published multi-trait fixtures, multi-seed calibration, and sommer/ASReml/BLUPF90 comparator parity",
