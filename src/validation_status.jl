const VALIDATION_STATUS_DATA = (
    ...
    (
        ...
        "LOCO defaults for a public workflow, marker-scan variance-component estimation, sparse production scans, calibrated mixed-model p-values, genomic-inflation diagnostics, interval-mapping or mixed-model LOD workflows, plotting backends, external comparator parity, and R-facing `marker_scan()` syntax",
        "Dense validation-scale LOCO construction and supplied-matrix selection helpers only; LOCO precision construction uses VanRaden-plus-ridge identities and the scan uses supplied variance components, but there are no public LOCO defaults, no variance-component estimation, no p-value calibration/genomic-inflation claim, no R formula activation, no bridge payload change, and no comparator evidence.",
    ),
    (
        "V5-GENOMIC-QTL",
        ...
        "model-spec contracts, formula-driven mixed-model marker scans, simulations, marker-map validation, public LOCO workflow defaults, genome-wide multiple-testing calibration, calibrated mixed-model p-values, genomic-inflation diagnostics, interval-mapping or mixed-model LOD workflows, QTL/eQTL intervals, and JWAS/sommer/BLUPF90-style comparators",
