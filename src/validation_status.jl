const VALIDATION_STATUS_DATA = (
    ...
    (
        "V6-GGLLVM-MARGINAL",
        "genetic-GLLVM non-Gaussian K-factor latent Laplace marginal (supplied loadings, per-trait families) (#50 slice 2 + extension)",
        "Phase 6",
        "partial",
        "`gllvm_laplace_marginal_loglik(Y, Ainv, loadings, family; X)` (internal) evaluates the Laplace-approximate marginal log-likelihood of the K-FACTOR genetic GLLVM: latent field `vec(g) ~ N(0, I_K ⊗ A)` (each factor `g[·,k] ~ N(0,A)` independently), `η[i,t] = (Xβ)[i,t] + Σ_k Λ[t,k] g[i,k]`, `y[i,t] | η ~ family` (`Gaussian`/`Poisson`/`Bernoulli`/`Binomial`), `β` integrated under a flat prior. It generalizes the single-factor `laplace_marginal_loglik` by replacing `(Z, Ainv/σ²a)` with `(W, I_K ⊗ Ainv)`, where `W` is the Λ-weighted latent design (record `(i,t)` scatters `Λ[t,:]` into animal `i`'s `K` factor slots) — penalized-IRLS Newton over `[β; vec(g)]` then a Gaussian integral at the mode. `family` now accepts a single `ResponseFamily` (applied uniformly to all traits) OR a length-`T` `Vector{ResponseFamily}` (one per trait column of `Y`); a per-record lookup is built from the vector, `_check_counts` is run per column against its own family, and the appropriate score/weight/loglik kernel is dispatched for each record. Validated (`test/runtests.jl`) by TWO INDEPENDENT trusted-path reductions: (1) the `K = 1, T = 1` case reduces EXACTLY to `laplace_marginal_loglik` with `σ²a = λ²` for BOTH the Gaussian (machine precision) and Poisson (rtol 1e-7) families (the Laplace approximation is invariant under the affine latent reparameterization `u = λg`); (2) the Gaussian, full-rank-`Λ` (`K = T`) case equals `_multivariate_reml_loglik` at `G0 = ΛΛ'`, `R0 = σ²e·I` (the Gaussian Laplace is exact, rtol 1e-7); plus Bernoulli/Binomial convergence with mode-stationarity (`‖∇‖ < 1e-8`) and dimension/count guards. The non-Gaussian K>1 loglik VALUE is anchored by a block-diagonal-`Λ` test (independent traits ⇒ the K=2 Poisson marginal equals the SUM of two single-factor marginals), and K>T / singular-`ΛΛ'` cases (handled because `P = I_K ⊗ Ainv` is full-rank regardless of `rank(ΛΛ')`) are pinned against `_multivariate_reml_loglik`. Per-trait family extension validated (8 assertions, `test/runtests.jl`): scalar-uniform-vector equality (EXACT same loglik bit-for-bit), mixed-family convergence (Poisson + Gaussian traits, finite loglik), wrong-length guard. A bounded 3-lens adversarial-verification Workflow (Gauss/Noether/Curie + Rose) returned `confirmed_correct` (the value re-derived from first principles + an independent FD Hessian).",
        "a fitted-object/EBV extractor surface, unbalanced / missing-record support, and external GLLVM.jl/gllvmTMB comparator parity",
        "Experimental, dense/validation-scale, SUPPLIED loadings (NOT estimated), balanced/fully-observed `Y` only; INTERNAL (not exported, mirroring the single-factor kernel's visibility), no R model-spec or bridge payload. The genuinely new genetic-GLLVM capability (a `K>1` latent field carrying a genetic relationship under a non-Gaussian response), validated by independent reductions but not by an external comparator; per-trait family dispatch now validated. `GLLVM-style animal models` stays `planned`. Nothing promoted to covered.",
    ),
    (
        "V6-GGLLVM-REML",
        ...
        "Phase 6",
        "partial",
        "`fit_gllvm_laplace_reml(Y, Ainv, family; rank, X)` (internal) ESTIMATES the rank-`K` latent loadings `Λ` (`G_lat = ΛΛ'`) by maximizing the K-factor Laplace marginal (`gllvm_laplace_marginal_loglik`) over `vec(Λ)` (NelderMead). The marginal depends on `Λ` only through `G_lat = ΛΛ'`, so it is rotation-invariant; the result reports the rotation-INVARIANT `genetic_covariance`/`latent_structure` (never the raw `Λ̂`). Validated (`test/runtests.jl`): the `K=1, T=1` Poisson case reduces to the single-factor `fit_laplace_reml` (`σ²a = λ̂²`, rtol 2e-3); a multi-trait Poisson rank-1 fit converges and its optimum improves over the start (objective maximized); and the Gaussian optimum's marginal equals `_multivariate_reml_loglik` at the estimated `Λ̂Λ̂'` (`R0 = σ²e·I`, rtol 1e-7) — self-consistency of the reported `G_lat`. A `structure = :factor_analytic` option additionally estimates a per-trait specific genetic variance `Ψ > 0` (`G_lat = ΛΛ' + diag(Ψ)`, on the `log` scale, by augmenting the loadings to `[Λ | diag(√Ψ)]` and reusing the marginal); validated by the Gaussian self-consistency at `Ĝ = Λ̂Λ̂'+Ψ̂`, `communality < 1` (since `Ψ̂ > 0`), `FA loglik ≥ low-rank loglik` (FA nests low-rank), a converging Poisson FA fit, and length/structure guards. For `GaussianResponse(σ²e)` the residual is the FIXED scalar `σ²e` (not estimated); the non-Gaussian families have no residual. An opt-in known-truth recovery harness (`sim/phase6_gllvm_recovery.jl`, outside CI) records POSITIVE recovery across TWO Poisson scenarios (`docs/dev-log/recovery-checkpoints/2026-06-20-genetic-gllvm-reml-recovery.md`): a rank-1 DGP (`Λ = [1.0, 0.7, 0.5]`, `q=240`) recovers `G_lat` at 5/5 seeds, mean `‖Ĝ−G‖/‖G‖ = 0.091`; a rank-2, NON-degenerate-correlation DGP (`q=120`) recovers at 5/5, mean rel `0.205` and genetic correlations `mean|Δρ| = 0.089` (loose gate `rel ≤ 0.45`) — graceful degradation as the structure hardens. POSITIVE but Poisson-only, balanced; NOT an FA(+Ψ)/Bernoulli/Binomial recovery and not an external comparator (recovery is on `G_lat`; rank-1 ⇒ `±1` correlations by construction). `fit_gllvm_laplace_reml` passes a single `family` to the marginal (the per-trait family extension of the marginal is available via `gllvm_laplace_marginal_loglik` directly; threading per-trait families through the REML estimator is noted as future work).",
        "BROAD recovery calibration (higher ranks, the FA structure, Bernoulli/Binomial, smaller `q`), a fitted-object/EBV extractor surface, threading per-trait families through the REML estimator, unbalanced/missing-record support, and external GLLVM.jl/gllvmTMB comparator parity",
        "Experimental, dense/validation-scale (low-rank + factor-analytic `G_lat`), one family for all traits (the marginal itself now supports per-trait families), balanced/fully-observed `Y`; INTERNAL (not exported). A correctness-validated ESTIMATOR (reductions + optimum-improvement + Gaussian self-consistency) WITH a first positive rank-1-Poisson recovery study — but NOT broadly calibrated and no external comparator, so `GLLVM-style animal models` stays `planned`. Nothing promoted to covered.",
    ),
)
