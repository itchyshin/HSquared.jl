const VALIDATION_STATUS_DATA = (
    ...
    (
        ...
        join((
            ...
            "The R lane records a 100-rep cold-start known-truth study (`hsquared/data-raw/multivariate-recovery-study.R`, 2026-06-20; `G0 = R0 = diag(2)` start, not truth) on a 420-animal two-trait pedigree: 100/100 converged; all 9 reported targets (six G0/R0 entries, rg, and two h² values) had 0 inside bias ± 2·MCSE; EBV accuracy was 0.790/0.742. This corroborates the Julia 12-seed bias/MCSE and cold-start result with tighter MCSE, but remains validation-scale recovery evidence, not external multi-package parity.",
            "A live external `sommer` 4.4.5 comparator run was reproduced on 2026-06-21 from the R-lane script `hsquared/data-raw/multivariate-comparator-study.R` against `test/fixtures/phase4_multitrait_parity/`, rebuilding `A` independently with `nadiv::makeA` (not copying Julia `Ainv`): max |dG0| = 7.529e-05, max |dR0| = 7.626e-06, max |dbeta| = 1.801e-06, max |dh2| = 6.821e-05, EBV correlations = 1.000/1.000, max |dEBV| = 4.398e-05; REML loglik is not compared because `sommer` and HSquared.jl use different additive-constant scales.",
        ...
        "additional independent same-estimand comparator parity (ASReml/BLUPF90/DMU/WOMBAT or equivalent; current same-estimand REML external evidence is one `sommer` fixture run, `MCMCglmm`/JWAS are Bayesian agreement only, and BLUPF90 is preflighted but not executed)",
        "Experimental dense/validation-scale multivariate REML; correctness is self-consistency + univariate-reduction validated and adversarial-reviewed (robustness findings fixed), Julia-side accessors wrap existing fields, the univariate `result_payload()` is unchanged and `multivariate_result_payload` now exposes a bridge-ready `:unstructured`/`:diagonal` payload (rotation-free; lowrank/fa loadings not bridged), opt-in Julia and R-lane cold-start recovery studies show no detectable bias at validation scale, and the serialized target plus comparator protocol now has one reproduced external `sommer` 4.4.5 same-estimand comparator leg, an R-lane Mrode Example 5.1 supplied-covariance BLUP/MME anchor, Bayesian `MCMCglmm` agreement evidence, and a tested BLUPF90 preflight harness; still partial because the executed per-seed calibration protocol did not pass, recovery is not coverage-calibrated, and there is no executed second same-estimand comparator run; not the public default, with R-facing opt-in multivariate bridge semantics still gated for public covered status.",
    ),
    (
        "V4-FA",
