const VALIDATION_STATUS_DATA = (
    ...
    (
        ...
        "Phase 5",
        "partial",
        "`single_marker_scan` residualizes `y` and centered marker dosages against `X`, then returns marker effects, supplied-variance standard errors, Wald z-scores, χ² statistics, approximate two-sided Gaussian/Wald p-values, Bonferroni-adjusted p-values, Benjamini-Hochberg q-values, and fixed-effect known-variance LOD-equivalent scores (`chisq / (2log(10))`). `marker_effects` prepares sorted top-marker effect summaries, with optional `HSMarkerMapSpec` / `HSData` chromosome-position alignment by exact marker ID. `marker_variance_explained` prepares sorted marker-level variance-contribution summaries as `2p(1-p) * effect^2`, with optional total-variance proportions and the same marker-metadata alignment. `marker_manhattan_data` prepares plot-ready marker IDs, chromosome labels, positions, cumulative plot positions, raw p-values, and `-log10(p)` values from direct scan output without drawing plots; it can also consume already-validated `HSMarkerMapSpec` / `HSData` marker metadata and align chromosome/position values by exact marker ID. `marker_qq_data` prepares QQ plot data with sorted observed p-values, expected uniform order-statistic p-values, sorted marker IDs, observed/expected `-log10` values, and p-floor display handling. `marker_genomic_inflation` computes a genomic-control-style λGC diagnostic from returned χ² values for fixed, mixed, and LOCO direct scans. Deterministic tests pin hand-computed scan effects (`17/14`, `0.5`), p-values, Bonferroni/BH adjustments, LOD-score identity, marker-effect summary ordering/top-N/metadata alignment, marker-variance summary identities/top-N/proportions/metadata alignment, Manhattan defaults, chromosome-offset ordering, marker-map-backed metadata alignment, Manhattan and QQ p-floor handling, QQ order/expected p-values, λGC median-χ² identities, covariate-adjusted scan consistency, default/supplied marker IDs, and guardrails for invalid residual variance, marker metadata lengths/values, duplicate or mismatched marker-map IDs, invalid p-value adjustment inputs, invalid genomic-inflation, marker-effect-summary, and marker-variance-summary inputs, collinear markers, rank-deficient `X`, row-count mismatch, and malformed QQ inputs.",
        "mixed-model marker scans with relationship correction, LOCO, QTL/eQTL intervals, calibrated mixed-model p-values, calibrated PVE or model R² claims, interval-mapping or mixed-model LOD workflows, actual plotting backends, advanced/correlated-marker multiple-testing workflows, external comparator parity, and R-facing `marker_scan()` syntax",
        "Fixed-effect Gaussian screening utility with marker-effect and marker-variance-contribution summaries, marker-map-backed Manhattan, QQ, and inflation diagnostic helpers only, using supplied residual variance plus approximate Wald p-values, Bonferroni/BH adjustments, LOD-equivalent scores, Manhattan data, QQ data, and a λGC diagnostic over the returned marker set; no mixed-model GWAS/QTL claim, no p-value calibration claim, no calibrated PVE or model R² claim, no R formula term activation, no bridge payload change, and no comparator evidence.",
    ),
    (
        "V5-MARKER-MIXED",
