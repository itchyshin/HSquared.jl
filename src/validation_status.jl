const VALIDATION_STATUS_DATA = (
    ...
    (
        ...
        "Phase 5",
        "partial",
        "`genome_wide_threshold_from_null(null_max_statistics; alpha, statistic)` turns a SUPPLIED empirical null distribution of per-scan maximum statistics into the `(1 - alpha)` empirical-quantile genome-wide threshold, and `genome_wide_pvalue(observed, null_max_statistics)` gives the add-one empirical genome-wide p-value `(1 + #{null ≥ observed})/(n_null + 1)`; the per-scan maximum is extracted by `_scan_max_statistic` (max chi-square or max -log10 p) and the quantile by the in-package type-7 `_empirical_upper_quantile` (dependency-light, no Statistics). Because it uses the distribution of the MAXIMUM over the jointly-scanned markers, the threshold is correlation/LD-aware (less conservative than Bonferroni under LD). Deterministic CI tests pin the max-statistic extraction (chi-square + -log10 p), the empirical-quantile interpolation against hand values, threshold shape + alpha-monotonicity, the add-one p-value identities (never zero), the threshold↔p-value consistency, and the input guards (`test/runtests.jl`). The RNG-heavy null generation is the opt-in `sim/phase5_threshold_calibration.jl` harness (outside CI): phenotype permutation (residual permutation conditional on `X`) re-runs the marker scan to build the null, contrasts the permutation threshold with Bonferroni, and records a loose empirical type-I smoke.",
