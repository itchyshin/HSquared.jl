const VALIDATION_STATUS_DATA = (
    ...
    (
        ...
        "Experimental dense/validation-scale engine API only; copy-returning structured-metadata accessors expose existing fields locally; returned loadings are sign-canonicalized but not rotation-identified, opt-in recovery evidence is internal and seeded, no R-facing covariance-structure syntax, no bridge/result-payload change, no production sparse FA solver, and no external comparator evidence.",
    ),
    (
        "V5-GENOMIC-QTL",
