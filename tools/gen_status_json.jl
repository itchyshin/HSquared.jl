...
#
# Honesty: public_covered_count is hard-pinned to 3 — (1) the v0.1 univariate Gaussian
...
# still experimental), and (3) the opt-in arbitrary-N independent random-effect model
# (`(1|g)`; `engine="julia", target="multi_effect"`; animal ratio covered, INDEPENDENT
# effects only, intervals asymptotic/uncalibrated). The covered/partial/planned split is
# generated, never quoted in prose.
...
status = """{
  ...
  "public_covered_count": 3,
  "honesty_assert": "Public-covered FITTING surface = 3: (1) v0.1 univariate Gaussian animal model (default path); (2) the opt-in common-environment two-effect model (engine=julia, target=two_effect; c² covered, maternal leg experimental); and (3) the opt-in arbitrary-N independent random-effect model ((1|g); target=multi_effect; animal ratio = narrow-sense h², other blocks are variance-explained proportions NOT heritabilities; INDEPENDENT effects only, NOT correlated/RR/non-Gaussian). All intervals asymptotic/delta-method, NOT coverage-calibrated. Everything else is experimental / partial / planned. Counts below are generated from validation_status(), never hand-typed.",
...
println("wrote ", joinpath(deploy, "status.json"), "  (rows=$rows covered=$cov, public_covered=3)")
